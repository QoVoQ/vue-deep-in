...
describe("Component scoped slot", () => {
  it("default slot", done => {
    const vm = new Vue({
      ...
      components: {
        test: {
          ...
          `,
          render(h) {
            return h("div", {}, this._t("default", null, {msg: this.msg}));
          }
        }
      }
    }).$mount();
    ...
    (vm.$children[0] as any).msg = "world";
    waitForUpdate(() => {
  ...
  it("named scoped slot", done => {
    const vm = new Vue({
      ...
      render(h) {
        return h("test", {
          scopedSlots: {
            item: props => [h("span", {}, props.foo), h("span", {}, props.bar)]
          }
        });
      },
      components: {
        test: {
          ...
          render(h) {
            return h(
              "div",
              {},
              this._t("item", null, {foo: this.foo, bar: this.bar})
            );
          }
        }
      }
        ...
        test: {
          ...
          render(h) {
            return h(
              "div",
              {},
              this._t("item", h("span", {}, [this.msg + " fallback"]), {
                test: this.msg
              })
            );
          }
  ...
  it("slot with v-for", done => {
        ...
        test: {
          ...
          render(h) {
            return h(
              "div",
              {},
              this._l(this.items, ele => this._t("item", null, {text: ele}))
            );
          }
        }
      }
    ...
    assertOutput();
    (vm.$children[0] as any).items.reverse();
    waitForUpdate(assertOutput)
      .then(() => {
        (vm.$children[0] as any).items.push("qux");
      })
      .then(assertOutput)
      .then(done);
  });
    ...
    const vm = new Vue({
      ...
      components: {
        test: {
          ...
          render(h) {
            const res = this._t("default", null, {msg: this.msg});
