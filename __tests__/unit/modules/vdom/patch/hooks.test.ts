...
import platformModules from "src/web/runtime/modules";
import {NodeOps as nodeOps} from "src/web/runtime/NodeOps";
import {createTextVNode, VNode} from "src/core/vdom/VNode";
import {VNodeHookNames} from "src/web/runtime/modules/definition";
...
describe("vdom patch: hooks", () => {
  ...
  it("should invoke `destroy` module hook for all removed children", () => {
    ...
    const patch1 = createPatchFunction({
      nodeOps,
      modules: (modules as any).concat([
        {
          [VNodeHookNames.create]() {
            created++;
          }
        },
        {
          [VNodeHookNames.destroy]() {
            destroyed++;
          }
        }
      ])
