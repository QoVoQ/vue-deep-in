...
describe("vdom domProps module", () => {
  it("should create an element with domProps", () => {
    const vnode = new VNode("a", {domProps: {href: "http://localhost/"}});
    const elm = patch(null, vnode) as HTMLAnchorElement;
    expect(elm.href).toBe("http://localhost/");
  });

  it("should change the elements domProps", () => {
    const vnode1 = new VNode("a", {domProps: {href: "http://localhost/"}});
    const vnode2 = new VNode("a", {domProps: {href: "https://vuejs.org/"}});
    patch(null, vnode1);
    const elm = patch(vnode1, vnode2) as HTMLAnchorElement;
    expect(elm.href).toBe("https://vuejs.org/");
  });

  it("should remove the elements domProps", () => {
    const vnode1 = new VNode("a", {domProps: {scopeId: "http://localhost/"}});
    const vnode2 = new VNode("a", {domProps: {}});
    patch(null, vnode1);
    const elm = patch(vnode1, vnode2) as HTMLAnchorElement;
    expect((elm as any).scopedId).toBeUndefined();
  });

  it("should initialize the elements value to zero", () => {
    const vnode = new VNode("input", {domProps: {value: 0}});
    const elm = patch(null, vnode) as HTMLInputElement;
  ...
  it("should save raw value on element", () => {
    ...
    const elm = patch(null, vnode) as HTMLInputElement;
    expect((<any>elm)._value).toBe(value);
  });
  ...
  it("should handle mutating observed props object", done => {
    ...
    (vm as any).props.id = "bar";
    waitForUpdate(() => {
      expect(vm.$el.id).toBe("bar");
      (vm as any).props = {id: "baz"};
    })
      .then(() => {
